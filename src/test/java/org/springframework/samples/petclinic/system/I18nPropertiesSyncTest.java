...
public class I18nPropertiesSyncTest {
	...
	public void checkNonInternationalizedStrings() throws IOException {
		...
		for (Path file : files) {
			...
			processLines(report, file, lines);
		}
	...
	private void processLines(StringBuilder report, Path file, List<String> lines) {
		for (int i = 0; i < lines.size(); i++) {
			String line = lines.get(i).trim();
			if (isIgnoredLine(line)) continue;

			if (file.toString().endsWith(".html")) {
				checkHtmlLine(report, file, line, i);
			}
		}
	}
	...
	private void checkHtmlLine(StringBuilder report, Path file, String line, int lineNumber) {
	...
	public void checkI18nPropertyFilesAreInSync() throws IOException {
		...
		checkMissingKeys(localeToProps, baseProps, baseFile);
	}
	...
	private void checkMissingKeys(Map<String, Properties> localeToProps, Properties baseProps, String baseFile) {
